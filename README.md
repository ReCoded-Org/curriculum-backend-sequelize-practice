...
Now let's run this file: `docker compose run -i app connect.js`. You should see the output as :
...
Once again, run `docker compose run -i app model.js`. You should see the query being executed in the output and the todo table will be created.
...
Note: You will have to add the SQL connection and model definition on each file, since we will be running each file independently. Example : `docker compose run -i app insert_todo.js`
